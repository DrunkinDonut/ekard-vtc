...
<html lang="pl">
	<head>
		...
		?>
		<title>Wszystkie trasy - EKARD VTC</title>
	</head>
	<body>
		...
		<nav>
			<div class="nav_section">
				...
				<ul>
					...
					<a href="all_routes" title="Wszystkie trasy" class="nav_item nav_active"><li>Wszystkie trasy</li></a>
				</ul>
			</div>
			<div class="nav_section">
		...
		<section>
			<div class="all_routes">
				<h1>Wszystkie trasy</h1>
				<div class="block_content">
					<div class="all_routes_table">
						<?php
							$allRoutes = mysqli_query($conn, "SELECT * FROM routes WHERE Id < 3 OR Id > 43 ORDER BY DateAndTime DESC");

							if(mysqli_num_rows($allRoutes) >= 1){
								echo "<div class='check_route_table'><table><thead><tr><th>ID</th><th>Dodano</th><th>Kierowca</th><th>Miejsce rozpoczęcia</th><th>Miejsce zakończenia</th><th>Ładunek</th><th>Dystans</th><th>Status</th><th>Sprawdzał</th><th></th></thead><tbody>";
								while($route = mysqli_fetch_array($allRoutes)){
									...
									echo "<tr><td>#EK".$route['Id']."</td><td>"; $addedDate = explode(" ", $route['DateAndTime']); echo $addedDate[0]."</td><td><a class='profile_link' href='profile?id=".$routeDriverId['Id']."' title='".$route['Login']."'>".$route['Login']."</a></td><td>".$route['CityFrom']."</td><td>".$route['CityTo']."</td><td>".$route['LoadName']." (".$route['Tonnage']." t)</td><td>".number_format($route['Distance'], 0, ',', ' ')." km</td><td><span class='status_class ".$statusClass."'>".$route['Status']."</span></td><td><a class='profile_link' href='profile?id=".$routeInfoCheckedId['Id']."' title='".$route['CheckedBy']."'><span style='color: #e1a01a;'>".$route['CheckedBy']."</span></a></td><td style='width: 180px;'><a class='check_route_button' href='route?id=".$route['Id']."' title='Przejrzyj trasę #EK".$route['Id']."'><span class='fas fa-search fa-sm'></span>Podgląd</a></td></tr>";
								}
								echo "</tbody></table></div>";
							}
							else{
								echo "<div style='font-size: 25px;' class='center'>Brak tras.</div>";
							}
						?>
					</div>
				</div>
